...
const serverConfig = type({
	...
	cookie_secret: type('string | null').pipe((v) => {
		if (v === null) return v;
		if (v.length !== 32)
			throw new Error('cookie_secret must be exactly 32 characters long');
		return v;
	}),
...
const oidcConfig = type({
	...
	headscale_api_key: type('string | null'),
	...
	.pipe((v) => {
		if ((v.client_secret == null) === (v.client_secret_path == null)) {
			throw new Error(
				'Must specify either client_secret or client_secret_path, but not both',
			);
		}
		if ((v.headscale_api_key == null) === (v.headscale_api_key_path == null)) {
			throw new Error(
				'Must specify either headscale_api_key or headscale_api_key_path, but not both',
			);
		}
		return v;
	})
